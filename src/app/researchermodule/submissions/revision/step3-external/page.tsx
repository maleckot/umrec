// app/researchermodule/submissions/revision/step3-external/page.tsx
'use client';
...
export default function RevisionStep3ExternalReviewed() {
  const router = useRouter();
  const isInitialMount = useRef(true);
  const saveTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [reviewerComments] = useState(
    'Please update your REC information if there have been any changes. If you have a revised Decision Letter or Research Ethics Clearance, please upload the updated document.'
  );
  ...
  useEffect(() => {
    const saved = localStorage.getItem('revisionStep3ExternalData');
    if (saved) {
      try {
        const parsedData = JSON.parse(saved);
        setFormData({ ...parsedData, uploadedFile: null });
      } catch (error) {
        console.error('Error loading Revision Step 3 External data:', error);
      }
    }
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      ...
      localStorage.setItem('revisionStep3ExternalData', JSON.stringify(dataToSave));
      console.log('💾 Revision Step 3 External auto-saved');
    }, 1000);
              ...
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm sm:text-base text-gray-600" style={{ fontFamily: 'Metropolis, sans-serif' }}>
                    Research Reviewed by External REC/Board
                  </p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
              ...
              <div>
                <label className="block text-sm sm:text-base font-bold mb-3 text-[#071139]" style={{ fontFamily: 'Metropolis, sans-serif' }}>
                  Upload the <span className="font-bold">Consolidated Files for Research Request</span> (Should include Decision Letter or Research Ethics Clearance). <span className="text-red-500">*</span>
                </label>
