  ...
  useEffect(() => {
    ...
    const fetchCommentsAndData = async () => {
      ...
      try {
        if (isQuickRevision && docId) {
          // ✅ QUICK REVISION: Fetch document verification feedback + revision count + submission comments
    ...
    `)
          ...
          const { data: submissionCommentsData } = await supabase
            .from('submission_comments')
            .select('comment_text, created_at')
            .eq('submission_id', submissionId) // ✅ NO document_id filter
            .eq('is_resolved', false)
            .order('created_at', { ascending: true });
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (isQuickRevision && submissionId && docId) {
    ...
    try {
      ...
      // Fetch old file URL and CURRENT revision count
  ...
  // ✅ NORMAL MULTI-STEP FLOW
    ...
    reader.onload = () => {
      ...
      setUploading(false);
      router.push(
        `/researchermodule/submissions/revision/step7?mode=revision&id=${submissionId}`
      );
    };
    reader.onerror = () => {
