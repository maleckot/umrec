...
import RevisionCommentBox from '@/components/researcher/submission/revision/step6/RevisionCommentBox';
import InstructionsSection from '@/components/researcher/submission/revision/step6/InstructionsSection';
import StudentGuidelines from '@/components/researcher/submission/revision/step6/StudentGuidelines';
import QualityRequirements from '@/components/researcher/submission/revision/step6/QualityRequirements';
import Step6Header from '@/components/researcher/submission/revision/step6/Step6Header';
...
function RevisionStep6Content() {
    ...
    const fetchCommentsAndData = async () => {
      ...
      try {
        if (isQuickRevision && docId) {
          ...
          const { data: docData } = await supabase.from('uploaded_documents').select('revision_count').eq('id', docId).single();
          setRevisionCount(docData?.revision_count || 0);

          const { data: submissionCommentsData } = await supabase.from('submission_comments').select('comment_text, created_at').eq('submission_id', submissionId).eq('is_resolved', false).order('created_at', { ascending: true });

          let combinedComments = '';
          if (verification?.feedback_comment) combinedComments += `**Document Verification Feedback:**\n${verification.feedback_comment}\n\n`;
          if (submissionCommentsData && submissionCommentsData.length > 0) {
            combinedComments += submissionCommentsData.map((comment, idx) => `**UMREC Comment ${idx + 1}:**\n${comment.comment_text}`).join('\n\n---\n\n');
          }
          setRevisionComments(combinedComments || 'No specific feedback provided. Please review the document for any general improvements.');
           ...
           `).eq('submission_id', submissionId).eq('status', 'submitted');
           ...
           if (submissionCommentsData && submissionCommentsData.length > 0) {
              ...
              combinedComments += '**UMREC Additional Comments:**\n\n' + submissionCommentsData.map((comment, idx) => `**Comment ${idx + 1}:** ${comment.comment_text}`).join('\n\n');
           }
           setRevisionComments(combinedComments || 'No reviewer comments available.');
        }
      } catch (error) { console.error(error); setRevisionComments('Unable to load feedback comments.'); } finally { setLoadingComments(false); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) { alert('Please upload a valid proposal defense certification document.'); return; }
    ...
    if (isQuickRevision && submissionId && docId) {
      ...
      try {
        ...
        if (existingDoc?.file_url) await supabase.storage.from('research-documents').remove([existingDoc.file_url]);
        ...
        const { error: updateError } = await supabase.from('uploaded_documents').update({ file_url: filePath, file_name: file.name, file_size: file.size, revision_count: newRevisionCount }).eq('id', docId);
        ...
        if (allDocs && allDocs.length > 0) {
             ...
             if (allAreNullOrApproved) await supabase.from('submission_comments').update({ is_resolved: true }).eq('submission_id', submissionId).eq('is_resolved', false);
        }
      ...
      } catch (error) { console.error('Error uploading:', error); alert('Failed to upload document. Please try again.'); } finally { setUploading(false); }
    } else {
      setUploading(true);
  ...
  const handleBack = () => {
    if (isQuickRevision && submissionId) router.push(`/researchermodule/activity-details?id=${submissionId}`);
    else router.push(`/researchermodule/submissions/revision/step5?mode=revision&id=${submissionId}`);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#E8EEF3] to-[#DAE0E7]">
      ...
      <div className="pt-24 md:pt-28 lg:pt-32 px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28 pb-8">
        <div className="max-w-[1400px] mx-auto">
          ...
          <Step6Header isQuickRevision={isQuickRevision} onBack={handleBack} />
            ...
            <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
              ...
              <div className="flex justify-end pt-8 mt-8 border-t-2 border-gray-200">
                <button
                  ...
                  className={`group relative px-10 sm:px-12 py-3 sm:py-4 rounded-xl font-bold text-base sm:text-lg shadow-xl hover:shadow-2xl hover:scale-105 overflow-hidden transition-all duration-300 ${!file || uploading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:from-orange-600 hover:to-orange-700'}`}
