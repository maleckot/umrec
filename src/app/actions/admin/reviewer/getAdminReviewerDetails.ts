// app/actions/staff/getReviewerDetails.ts (update the existing one)
'use server';
...
export async function getReviewerDetails(reviewerId: string) {
  try {
    ...
    const { data: reviewer, error: reviewerError } = await supabase
      .from('profiles')
      .select('id, full_name, email, phone, college, reviewer_code, availability_status, expertise_areas')
      .eq('id', reviewerId)
      .eq('role', 'reviewer')
      .single();
    ...
    const assignmentsWithSubmissions = assignments?.map(assignment => {
      ...
      return {
        ...assignment,
        research_submission: submission,
      };
    }) || [];
    ...
    return {
      ...
      reviewer: {
        ...
        areasOfExpertise: reviewer.expertise_areas || 'Not specified',
