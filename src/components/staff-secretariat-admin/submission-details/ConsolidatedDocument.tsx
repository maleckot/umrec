...
export default function ConsolidatedDocument({ 
  ...
  // ✅ Filter by name - remove Form 0011, Form 0012, and Certificate of Approval
  const filteredOriginalDocuments = (originalDocuments || []).filter((doc: any) => {
    const docName = typeof doc === 'string' ? doc : (doc.name || '');
    const nameLower = docName.toLowerCase();
    
    // Remove these files
    return !nameLower.includes('form 0011') && 
           !nameLower.includes('form 0012') &&
           !nameLower.includes('certificate of approval');
  });
        ...
        <div className="p-3 sm:p-4 lg:p-6">
          ...
          <div className="border-2 border-blue-500 rounded-lg overflow-hidden bg-blue-50">
            <div
              ...
              className="w-full flex items-center justify-between p-3 sm:p-4 hover:bg-blue-100 transition-colors gap-2 cursor-pointer"
            >
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                ...
                <div className="text-left min-w-0 flex-1">
                  ...
                  <span className="text-xs text-blue-700" style={{ fontFamily: 'Metropolis, sans-serif' }}>
                    {filteredOriginalDocuments.length} document{filteredOriginalDocuments.length !== 1 ? 's' : ''}
                  </span>
                </div>
              </div>
          ...
          {filteredOriginalDocuments.length > 0 && (
            <div className="mt-3 sm:mt-4">
              ...
              >
                {showOriginals ? '− Hide' : '+ Show'} Original Documents ({filteredOriginalDocuments.length})
              </button>

              {showOriginals && (
                <div className="mt-2 sm:mt-3 space-y-1.5 sm:space-y-2 pl-3 sm:pl-4 border-l-2 border-gray-300">
                  {filteredOriginalDocuments.map((doc, index) => {
                    const docName = typeof doc === 'string' ? doc : (doc.name || 'Unknown');
                    return (
                      <div key={index} className="text-xs text-gray-600 flex items-start gap-2" style={{ fontFamily: 'Metropolis, sans-serif' }}>
                        ...
                        <span className="flex-1 break-words">{docName}</span>
                      </div>
                    );
                  })}
                </div>
